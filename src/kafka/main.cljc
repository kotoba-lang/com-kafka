(ns kafka.main
  "Kotodama WASM entrypoint for the Kafka clean-room actor (L4) — Clojure port.
  ...
  facts (`kafka.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Topic"          :plural "topics"          :id-prefix "kafka_top"
    :fields [:name :numPartitions :replicationFactor :isInternal :minInsyncReplicas] :required [:name :numPartitions]
    :coerce {:numPartitions :int :replicationFactor :int :isInternal :bool :minInsyncReplicas :int} :refs {}}
   {:entity "Partition"      :plural "partitions"      :id-prefix "kafka_par"
    :fields [:topicName :partitionIndex :leader :replicas :isr] :required [:topicName :partitionIndex]
    :coerce {:partitionIndex :int :leader :int} :refs {}}
   {:entity "AclBinding"     :plural "aclbindings"     :id-prefix "kafka_acl"
    :fields [:resourceType :resourceName :principal :host :operation :permissionType] :required [:resourceType :resourceName]
    :coerce {}                       :refs {}}
   {:entity "ConsumerGroup"  :plural "consumergroups"  :id-prefix "kafka_con"
    :fields [:groupId :protocolType :state :generationId] :required [:protocolType :state]
    :coerce {:generationId :int}  :refs {}}
   {:entity "FetchRequest"   :plural "fetchrequests"   :id-prefix "kafka_fet"
    :fields [:topicName :partitionIndex :fetchOffset :maxBytes :isolationLevel] :required [:topicName :partitionIndex]
    :coerce {:partitionIndex :int :fetchOffset :int :maxBytes :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"kafka.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kafka-compat" :tier tier :entities entities} 200])
